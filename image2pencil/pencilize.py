...
def loadVOCImage(voc_path, img_name):
    # get the path of masks and images
    seg_path = voc_path + 'SegmentationClass/'
    img_path = voc_path + 'JPEGImages/'

    mask_name = seg_path + img_name + '.png'
    img_name = img_path + img_name + '.jpg'

    assert os.path.exists(mask_name), 'ERROR: This image might not have a corresponding segmentation, please check'.format(mask_name)
    ...
    _, bin_mask_img = cv.threshold(mask_img, 1, 255, cv.THRESH_BINARY)  # make multi gray level image to binary image
    ...
    # 'bin_mask' and 'bin_background' is contrast relation, both in 3-channels
    return color_img, bin_mask_img, bin_background_img


def image2pencil(voc_path, img_name, kernel=15, gamma_val=2.0):
    # =====================================
    # 1. load + crop + resize + adjust ratio
    # =====================================

    # get the images
    color_img, bin_mask_img, bin_background_img = loadVOCImage(voc_path, img_name)
